...
function GraficoAvanzadoPromedioBuses({ data, nombreFranjaMap, onClose }) {
  ...
  if (datosComparar && Array.isArray(datosComparar)) {
    // Mapear los datos de comparación a las horas del eje X
    const dataComp = horas.map(hora => {
      const found = datosComparar.find(d => d.hora === hora);
      return found ? found.servicios : null;
    });
    datasets.push({
      label: `Servicios en ${fechaComparar}`,
      data: dataComp,
      borderColor: 'green',
                    ...
                    tooltip: {
                      callbacks: {
                        ...
                        label: function (context) {
                          ...
                          // Buscar el promedio y la comparación para esta hora
                          const datasets = context.chart.data.datasets;
                          let promedio = null, comparar = null;
                          datasets.forEach(ds => {
                            if (ds.label === 'Promedio de selección') promedio = ds.data[idx];
                            if (ds.label && ds.label.startsWith('Servicios en')) comparar = ds.data[idx];
                          });
                          let msg = '';
                          if (label === 'Promedio de selección') {
                            msg = `Promedio: ${Number(value).toFixed(2)} buses`;
                          } else if (label.startsWith('Servicios en')) {
                            msg = `Comparación: ${Number(value).toFixed(2)} buses`;
                            if (promedio !== null && promedio !== undefined) {
                              const variacion = promedio === 0 ? 0 : ((value - promedio) / promedio) * 100;
                              msg += `\nPromedio: ${Number(promedio).toFixed(2)} buses`;
                              msg += `\nVariación: ${variacion.toFixed(2)}%`;
                            }
                          } else if (label.startsWith('Mínimo Res.') || label.startsWith('Esencialidad')) {
                            // No mostrar tooltip para la línea de referencia ni esencialidad
                            return null;
                          } else {
                            msg = `${label}: ${Number(value).toFixed(2)}`;
                          }
                          return msg;
                        }
                      }
