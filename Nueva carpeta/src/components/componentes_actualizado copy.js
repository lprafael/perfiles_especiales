...
function MiPaginaExistente() {
  ...
  const calcularPuntosDeControl = () => {
    ...
    shapes.forEach((shape) => {
      ...
      // const inicio = latlngs[0]; // <- Eliminar duplicado
      // const fin = latlngs[latlngs.length - 1]; // <- Eliminar duplicado
                  ...
                  if (idxTerminales.length >= 2) {
                    ...
                    for (let t1 = 0; t1 < idxTerminales.length; t1++) {
                      for (let t2 = 0; t2 < idxTerminales.length; t2++) {
                        ...
                        let ultimoDentroInicio = null;
                        let enInicio = false;
                        for (let i = 0; i < recorrido.length; i++) {
                          const p = recorrido[i];
                          // ¿Está dentro del punto de control de inicio?
                          if (dentroGeocerca(p, puntosDeControl[idxInicioPC], puntosDeControl[idxInicioPC].radius)) {
                            ultimoDentroInicio = { idx: i, punto: { ...p } };
                            enInicio = true;
                          } else {
                            if (enInicio) {
                              // Salió del punto de control de inicio
                              enInicio = false;
                            }
                          }
                          // ¿Está dentro del punto de control de fin?
                          if (dentroGeocerca(p, puntosDeControl[idxFinPC], puntosDeControl[idxFinPC].radius)) {
                            // Solo registrar trayecto si hay una última salida previa del inicio
                            if (ultimoDentroInicio && ultimoDentroInicio.idx < i) {
                              // El trayecto es desde la última salida del inicio hasta la entrada al fin
                              trayectos.push({
                                bus_id: bus.mean_id,
                                ...
                                recorrido: recorrido.slice(ultimoDentroInicio.idx, i + 1),
                              });
                              // Reiniciar para buscar el siguiente trayecto
                              ultimoDentroInicio = null;
                            }
                          }
                        }
                      }
              ...
              <div style={{ padding: 16, width: "100%" }}>
                ...
                {(infoServicios && (infoServicios.tipo === 'buses' || infoServicios.tipo === 'buses-tiempo-real') && (!simulacionEstado || (simulacionEstado && !simulacionEstado.corriendo))) ? (
                  ...
                  <div>
                    ...
                    <div style={{marginTop:16, textAlign:'center'}}>
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (!win) return;
                          win.document.write('<html><head><title>Previsualización de Reporte</title></head><body>');
                          win.document.write('<h2>Reporte de Servicios</h2>');
                          win.document.write(`<b>Empresa:</b> ${infoServicios.empresaNombre}<br/>`);
                          win.document.write(`<b>Fecha:</b> ${infoServicios.fecha}<br/>`);
                          win.document.write(`<b>Shapes cargados:</b> ${infoServicios.shapes?.length}<br/>`);
                          win.document.write('<b>Servicios detectados:</b><ul>');
                          win.document.write(`<li><b>Directos:</b> ${infoServicios.servicios_detectados?.directos}</li>`);
                          win.document.write(`<li><b>Circulares:</b> ${infoServicios.servicios_detectados?.circulares}</li>`);
                          win.document.write(`<li><b>Total:</b> ${infoServicios.servicios_detectados?.total}</li>`);
                          win.document.write('</ul>');
                          win.document.write('<b>Puntos de control:</b><ul>');
                          win.document.write(`<li><b>Total:</b> ${infoServicios.puntos_control?.total}</li>`);
                          win.document.write(`<li><b>Terminales:</b> ${infoServicios.puntos_control?.terminales}</li>`);
                          win.document.write(`<li><b>Intermedios:</b> ${infoServicios.puntos_control?.intermedios}</li>`);
                          win.document.write('</ul>');
                          ...
                          if (Array.isArray(infoServicios.trayectos) && infoServicios.trayectos.length > 0) {
                            ...
                            win.document.write('<div style="color:#888">No hay trayectos detectados.</div>');
                          }
                          win.document.write('<button onclick="window.opener && window.opener.guardarReporteWord && window.opener.guardarReporteWord()" style="margin-top:18px;padding:8px 18px;background:#388e3c;color:#fff;border:none;border-radius:5;font-weight:bold;font-size:16;cursor:pointer;">Guardar en Word</button>');
                    ...
                    {/* Función global para guardar en Word */}
                    <script>
                      {`
                        window.guardarReporteWord = function() {
                          alert('Funcionalidad de exportar a Word pendiente de implementación.');
                        }
                      `}
                    </script>
                  </div>
