...
function MiPaginaExistente() {
    ...
    fetch(`${API_BASE}/empresas/${empresaId}/itinerarios`)
      ...
      .then((data) => {
        ...
        data.forEach((it, index) => {
          ...
          // Color de fondo según si tiene shape
          if (it.shape_points && it.shape_points.length > 1) {
            li.style.backgroundColor = "#d4f7d4"; // verde claro
          } else {
            li.style.backgroundColor = "#ffe4e1"; // rosado claro o cualquier otro
          }

          // Dibujar shape si corresponde
          if (it.shape_points && it.shape_points.length > 1) {
            const latlngs = it.shape_points.map((p) => [p.lat, p.lng]);
            const color = colors[index % colors.length];
            const shape = L.polyline(latlngs, { color, weight: 4 }).bindPopup(
              li.textContent
            );
            ...
            if (window.turf) {
              const line = {
                ...
                geometry: {
                  type: "LineString",
                  coordinates: it.shape_points.map((p) => [p.lng, p.lat]),
                },
              };
              const buffered = window.turf.buffer(line, 0.05, { units: "kilometers" });
              ...
              // turf.js no está cargado
              console.warn("turf.js no está disponible. No se puede dibujar el buffer poligonal.");
            }
          }
          ...
          li.onclick = () => {
            ...
            const shape = group.getLayers()[index];
            if (shape) mapInstance.current.fitBounds(shape.getBounds());
          };
  ...
  function verificarSeleccion() {
    ...
    if (empresaId && fecha) {
      // --- POST al backend con los datos solicitados ---
      ...
      fetch(`${API_BASE}/procesar_seleccion`, {
        ...
        .then((res) => res.json())
        .then((data) => {
          // 4. Mostrar alertas con la respuesta del backend
          alert(
            `Usted seleccionó la empresa "${empresaNombre}" y la fecha ${fecha}\n\nRespuesta del backend: ${data.mensaje}`
          );
          // Mostrar la cantidad de servicios calculados
          const cantidadServicios = data.cantidad_servicios || 0;
          const mensaje = `Usted seleccionó la empresa "${empresaNombre}" y la fecha ${fecha}\n\nCantidad de servicios: ${cantidadServicios}`;
          alert(mensaje);
          mostrarAviso("Datos enviados correctamente al backend", "success");
        })
        .catch((err) => {
          // 5. Manejo de errores en la petición
          mostrarAviso("Error al enviar datos al backend", "error");
        });
    } else {
      // Si falta empresa o fecha, mostrar alerta
  ...
  return (
    <div className="container" style={{ border: "2px solid blue" }}>
      ...
      <div className="content">
        ...
        {/* <div className="main-content" style={{ border: "2px solid blue", position: "relative" }}></div> */}
