...
function RegularidadBusesModal(props) {
  ...
  React.useEffect(function() {
    ...
    if (modoSistema) {
      setLoading(true);
      setError(null);
      fetch(`http://192.168.100.191:8000/sistema/buses_regularidad_por_hora_agregado?fecha=${fecha}`)
        .then(res => res.json())
    ...
    let url = modoGremio && gremioId
      ? `http://192.168.100.191:8000/gremios/${gremioId}/buses_regularidad_por_hora_agregado?fecha=${fecha}`
      : `http://192.168.100.191:8000/empresas/${empresaId}/buses_regularidad_por_hora?fecha=${fecha}`;
    fetch(url)
      .then(res => res.json())
      .then(json => setData(json))
      .catch(() => setError('Error al obtener datos'))
  ...
  React.useEffect(function() {
    ...
    // Calcular el 60% del promedio histórico por hora
    var sesentaPorcientoPorHora = promedioPorHora.map(function(v) { return v * 0.6; });
    ...
    window._regularidadBusesChart = new window.Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          ...
          // --- Línea roja del 60% del promedio histórico ---
          {
            label: 'Límite mínimo (60% promedio histórico)',
            data: sesentaPorcientoPorHora,
            borderColor: 'red',
  ...
  return (
    ...
    >
      <div style={{
        ...
        <button onClick={onClose} style={{position:'fixed',top:18,right:18,fontSize:22,background:'#e3f2fd',color:'#1976d2',border:'2px solid #90caf9',borderRadius:20,width:'auto',height:48,padding:'0 24px',cursor:'pointer',zIndex:3001,boxShadow:'0 2px 8px #0002',fontWeight:'bold',display:'flex',alignItems:'center',gap:8}}>
          ← Volver
        </button>
