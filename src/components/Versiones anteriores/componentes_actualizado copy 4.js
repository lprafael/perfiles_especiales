...
function MiPaginaExistente() {
  ...
  const [mostrarSidebarIzquierdo, setMostrarSidebarIzquierdo] = useState(false); // Ahora inicia oculto
  ...
  // Estado para el checkbox de validaciones
  const [incluirValidaciones, setIncluirValidaciones] = useState(false);

  // Estado para los puntos de validación
  const [validaciones, setValidaciones] = useState([]);
  const validacionesLayer = useRef(null);

  // Cache para almacenar las líneas procesadas
  let shapesCache = {
    lines: [],
    radius: 0,
    lastUpdate: 0
  ...
  // Función para actualizar el cache de shapes
  function updateShapesCache() {
    if (!shapeLayer.current) return;
  ...
  function estaEnBuffer(lat, lng) {
    if (!turf || !shapeLayer.current) return false;

    // Actualizar cache si cambió el radio o no existe
    if (shapesCache.radius !== geocercaRadio || shapesCache.lines.length === 0) {
      updateShapesCache();
    }

    // Crear el punto una sola vez
    const pt = turf.point([lng, lat]);

    // Usar el cache de líneas
    const bufferKm = geocercaRadio / 1000;
    let minDistance = Infinity;

    // Buscar la distancia mínima usando el cache
    for (const line of shapesCache.lines) {
      try {
        const distance = turf.pointToLineDistance(pt, line, { units: 'kilometers' });
        if (distance <= bufferKm) {
          return true; // Retornar inmediatamente si encontramos una línea cercana
        }
  ...
  // Función para cargar buses en tiempo real de manera más eficiente
  async function cargarBusesTiempoReal(empresaId) {
    if (!empresaId) {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/empresas/${empresaId}/ultimos_gps?desde=${haceUnaHora.toISOString()}&hasta=${ahora.toISOString()}&solo_ultimo=1`
      );
  ...
  // Función optimizada para pintar buses
  function pintarBuses(buses, busesSeleccionados = []) {
    if (!mapInstance.current) return;

    // Limpiar capa previa
    if (window.busesLayer) {
      mapInstance.current.removeLayer(window.busesLayer);
    }
    window.busesLayer = L.layerGroup().addTo(mapInstance.current);

    // Filtrar buses seleccionados
    const busesFiltrados = busesSeleccionados.length === 0 ?
      buses :
      buses.filter(bus => busesSeleccionados.includes(bus.mean_id));
    ...
    // Procesar buses en lotes para mejor rendimiento
    const BATCH_SIZE = 50;
    for (let i = 0; i < busesFiltrados.length; i += BATCH_SIZE) {
      setTimeout(() => {
        const batch = busesFiltrados.slice(i, i + BATCH_SIZE);
        batch.forEach(bus => {
      ...
      }, Math.floor(i / BATCH_SIZE) * 10); // Pequeño delay entre lotes
    }

    mostrarAviso(`Se mostraron ${busesFiltrados.length} buses en tiempo real`, "success");
  }
            ...
            if (hasLines) {
              ...
              it.shape_lines.forEach((line, idx) => {
                ...
                if (window.turf) {
                  ...
                  const buffered = window.turf.buffer(lineFeature, geocercaRadio / 1000, { units: "kilometers" });
  ...
  }, [setMostrarSidebarDerecho, setSimulacionEstado]);

  // Modificar el useEffect para tiempo real
  useEffect(() => {
    if (busesTiempoRealActivo) {
      const actualizarBuses = async () => {
        if (!empresaId) return;

        try {
          const buses = await cargarBusesTiempoReal(empresaId);
          setBusesTiempoReal(buses);
          setBusesTiempoRealSeleccionados(prev => prev.length ? prev : buses.map(b => b.mean_id));
          pintarBuses(buses, busesTiempoRealSeleccionados);
          // Si está activado incluirValidaciones, cargar y mostrar validaciones
          if (incluirValidaciones) {
            await cargarValidaciones(empresaId, null, true);
          ...
          console.error('Error updating buses:', e);
        }
      };
      ...
      const interval = setInterval(actualizarBuses, 10000);
      return () => {
  ...
  }, [busesTiempoRealActivo, empresaId, geocercaRadio, busesTiempoRealSeleccionados, incluirValidaciones]);

  // Función para cargar y mostrar validaciones en el mapa
  async function cargarValidaciones(empresaId, fecha, modoTiempoReal) {
    // 1. Buscar cod_catalogo de la empresa
    let cod_catalogo = null;
    try {
      const resEots = await fetch(`${API_BASE}/eots?empresaId=${empresaId}`);
      const eots = await resEots.json();
      cod_catalogo = eots?.cod_catalogo;
    } catch (e) {
      mostrarAviso('Error al buscar cod_catalogo', 'error');
      return;
    }
    if (!cod_catalogo) return;
    // 2. Buscar distinct(id_sam) de declaración_jurada
    let ids_sam = [];
    try {
      let urlDJ = `${API_BASE}/declaracion_jurada?cod_catalogo=${cod_catalogo}`;
      if (modoTiempoReal) {
        urlDJ += '&max_fecha=1';
      } else if (fecha) {
        urlDJ += `&fecha=${fecha}`;
      }
      const resDJ = await fetch(urlDJ);
      const djs = await resDJ.json();
      ids_sam = djs?.ids_sam || [];
    } catch (e) {
      mostrarAviso('Error al buscar declaración jurada', 'error');
      return;
    }
    if (!ids_sam.length) return;
    // 3. Buscar última validación de cada id_sam en c_transacciones
    let puntos = [];
    try {
      const resVal = await fetch(`${API_BASE}/validaciones?ids_sam=${ids_sam.join(',')}&${modoTiempoReal ? 'max=1' : `fecha=${fecha}`}`);
      puntos = await resVal.json();
    } catch (e) {
      mostrarAviso('Error al buscar validaciones', 'error');
      return;
    }
    setValidaciones(puntos);
    // Pintar en el mapa
    if (validacionesLayer.current && mapInstance.current) {
      mapInstance.current.removeLayer(validacionesLayer.current);
    }
        ...
        <div className="sidebar" style={{ border: "2px solid blue" }}>
          ...
          >
            ...
            <button
              onClick={() => {
                if (busesTiempoRealActivo) {
                  setBusesTiempoRealSeleccionados(busesTiempoReal.map(b => b.mean_id));
                }
                setBusesTiempoRealActivo(act => !act);
              }}
                        ...
                        {infoServicios.buses.map((bus, idx) => (
                          <li key={bus.mean_id} style={{ marginBottom: 4 }}>
                            <input
                              ...
                              style={{ marginRight: 8 }}
                            />
                            <b>{bus.mean_id}</b>{bus.recorrido ? ` (${bus.recorrido.length} puntos)` : ''}
                          </li>
                        ))}
                      </ul>
                    </div>
